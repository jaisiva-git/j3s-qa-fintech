Feature: API error handling and (placeholder) auth tests
  ...
  Scenario: Insufficient funds
    Given I create a user with name "Low" email "low@j3s.com" type "standard"
    And I create a user with name "Rich" email "rich@j3s.com" type "standard"
  ...
  Scenario: Placeholder auth test (no auth required in sample backend)
    When I create a user with name "NoAuth" email "noauth@j3s.com" type "standard"
